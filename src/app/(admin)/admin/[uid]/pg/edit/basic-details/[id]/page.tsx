...
import PGFormSkeleton from "@/components/FormSkeletons/PgFormSkeleton";
        ...
        >
          {({ dirty, errors, values }) => {
            async function handleClose() {
              router.back();
              try {
                const prevImageIds: string[] =
                  initialFieldData?.pg_images?.map(
                    (img: any) => img?.pg_image_id
                  ) || [];
                const publicIdsForPgImages = values.pg_images
                  .filter(
                    (image: any) =>
                      image?.pg_image_id !== "" &&
                      !prevImageIds.includes(image.pg_image_id)
                  )
                  .map((image: any) => image?.pg_image_id);
                const payload = {
