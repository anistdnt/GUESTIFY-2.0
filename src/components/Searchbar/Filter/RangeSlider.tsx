...
// Generate scale marks positions outside the component to avoid re-creation on each render
const scaleMarks = [];
for (let i = MIN; i <= MAX; i += STEP) {
  scaleMarks.push(
    <div
      ...
      style={{ left: `${((i - MIN) * 100) / (MAX - MIN)}%`, transform: 'translateX(-1px)' }}
    />
  );
}

export const RangeSlider = ({ values, setValues }) => {
          ...
          return (
            <div
              ...
              className={`
                ...
                cursor-pointer
                transition-colors
                ${isDragged ? 'bg-orange-600' : 'hover:bg-orange-600'}
              `}
            >
